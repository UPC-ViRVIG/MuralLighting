...
# Update this to point to the local FastAPI route we are about to create
NODE_BASE_URL = "/asgi/public"
...
# This correctly points to the 'menu' folder next to main.py
...
@app.get('/app/images')
def get_images():
    textures_dir = os.path.join(public_path, 'textures')
    exr_files = []
    
    if os.path.exists(textures_dir):
        # Walk through the directory just like fs.readdirSync did
        for root, dirs, files in os.walk(textures_dir):
            for file in files:
                if file.endswith('.exr'):
                    # Get the relative path starting from 'textures/'
                    rel_dir = os.path.relpath(root, textures_dir)
                    rel_dir = "" if rel_dir == "." else rel_dir + "/"
                    exr_files.append(rel_dir + file)
                    
    return JSONResponse(content=exr_files)
...
if __name__ in {"__main__", "__mp_main__"}:
    # 1. Check if we are on the production server
    is_production = os.environ.get("ENV") == "production"
    
    # 2. Assign port (Default to 8000 in prod, 8080 locally)
    port = int(os.environ.get("PORT", 8000 if is_production else 8080))
    
    # 3. Security: In prod with Nginx, bind ONLY to localhost so the outside 
    # world can't bypass Nginx. Locally, bind to 0.0.0.0 so you can test on your phone.
    host = "127.0.0.1" if is_production else "0.0.0.0"

    # Generate a random secret if one isn't provided by the server environment
    default_secret = secrets.token_urlsafe(32)
    secret = os.environ.get("STORAGE_SECRET", default_secret)

    ui.run(
        title="Mural Lighting", 
        storage_secret=secret,
        host=host,
        port=port,
        reload=not is_production,
        show=not is_production
    )